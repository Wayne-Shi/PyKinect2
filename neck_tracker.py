...
class BodyGameRuntime(object):
    ...
    def draw_body_joint(self, joints, jointPoints, color, joint0):
        joint0State = joints[joint0].TrackingState;
        ...
        # ok, at least one is good
        dot = (jointPoints[joint0].x, jointPoints[joint0].y)

        try:
            pygame.draw.circle(self._frame_surface, color, dot, radius=50, width= 10)
        except: # need to catch it due to possible invalid positions (with inf)
            pass

    def show_the_alert(self, text, state):
    ...
    def detect_joint_angle(self, joints, jointPoints, joint0, joint1, joint2, joint3):
        joint0State = joints[joint0].TrackingState;
        joint1State = joints[joint1].TrackingState;
        joint2State = joints[joint2].TrackingState;
        joint3State = joints[joint3].TrackingState;

        data_available = True
        # joint is not tracked
        if joint0State == PyKinectV2.TrackingState_NotTracked\
                or joint1State == PyKinectV2.TrackingState_NotTracked\
                or joint2State == PyKinectV2.TrackingState_NotTracked\
                or joint3State == PyKinectV2.TrackingState_NotTracked:
            data_available = False

        # joint is not *really* tracked
        if joint0State == PyKinectV2.TrackingState_Inferred\
                or joint1State == PyKinectV2.TrackingState_Inferred\
                or joint2State == PyKinectV2.TrackingState_Inferred\
